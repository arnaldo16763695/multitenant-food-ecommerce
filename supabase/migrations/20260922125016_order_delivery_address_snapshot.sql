-- Recreate the atomic order-creation RPC so delivery orders persist a real address snapshot and
-- fee instead of the '{}'::jsonb placeholder / hardcoded 0 total this function has used since it
-- was written (nothing has ever sent fulfillment_type = 'delivery' until now). The two new
-- parameters are appended at the end with defaults so any other caller stays compatible. Body is
-- otherwise unchanged from 20260904194354_order_item_combo_components.sql (items/modifiers/combo
-- component loops, order_status_history insert, return shape all identical).
...
begin
  ...
  values (
    ...
    0,
    p_subtotal + coalesce(p_delivery_fee, 0),
    'MXN',
    p_customer_notes
  )
