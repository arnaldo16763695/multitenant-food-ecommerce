...
set from_status = 'fulfilled'
where from_status = 'completed';

update public.order_status_history
set to_status = 'fulfilled'
...
check (status in ('pending_payment', 'confirmed', 'in_preparation', 'ready', 'fulfilled', 'cancelled'));
...
begin
  ...
  set
    ...
    completed_at = case when p_to_status = 'fulfilled' then now() else completed_at end,
