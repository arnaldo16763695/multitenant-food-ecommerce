...
import { replaceCustomerBagItemConfigurations } from "@/lib/services/customer-bag"

type MobileBagItemConfigurationsItemRouteContext = {
...
// Splits the bag line identified by bagItemId into the given configurations -- e.g. editing a
// line of 3 into "2 sin cebolla" + "1 sin mostaza" produces two independent bag lines. At least
// one configuration with quantity >= 1 is required; to remove the line entirely use
// DELETE /bag/items/{bagItemId} instead of this endpoint.
...
export async function PATCH(request: Request, context: MobileBagItemConfigurationsItemRouteContext) {
  ...
  const result = await replaceCustomerBagItemConfigurations(authResult.adminClient, {
    bagItemId,
    tenantSlug,
    branchId,
  ...
  if (!result.ok) {
    return mobileError(400, result.error ?? "No pudimos actualizar las combinaciones de la bolsa.")
  }
