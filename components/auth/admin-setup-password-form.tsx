...
export function AdminSetupPasswordForm({ nextPath }: AdminSetupPasswordFormProps) {
  ...
  const [infoMessage, setInfoMessage] = React.useState("Validando tu acceso...")
  ...
  React.useEffect(() => {
    ...
    async function bootstrapSession() {
      ...
      if (isMounted) {
        setIsReady(true)
        setInfoMessage("Define tu contraseña para usar este acceso todos los dias.")
      }
    }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    router.replace(`/auth/admin/login?next=${encodeURIComponent(nextPath)}&reason=password-set`)
    router.refresh()
  }
  ...
  return (
    <Card className="w-full max-w-md border-stone-200 shadow-[0_20px_70px_rgba(28,25,23,0.08)]">
      <CardHeader>
        <CardTitle>Activa tu acceso</CardTitle>
        <CardDescription>{errorMessage ? "Necesitamos completar tu acceso antes de entrar al panel." : infoMessage}</CardDescription>
      </CardHeader>
      <CardContent>
        <form className="grid gap-4" onSubmit={handleSubmit}>
          <label className="grid gap-2 text-sm">
            <span className="font-medium text-card-foreground">Nueva contraseña</span>
          ...
          <label className="grid gap-2 text-sm">
            <span className="font-medium text-card-foreground">Confirmar contraseña</span>
            <Input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Repite tu contraseña" disabled={!isReady || isSubmitting} required />
          </label>
